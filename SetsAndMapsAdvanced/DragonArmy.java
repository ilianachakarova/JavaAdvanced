...
public class DragonArmy {
    public static void main(String[] args) {
        ...
        dragons.entrySet().forEach(entry ->{
            ...
            avgDamage = aggregatedData[0] /( dragonsCount * 1.00);
            avgHealth = aggregatedData[1] / ( dragonsCount * 1.00);
            avgArmor = aggregatedData[2] / ( dragonsCount * 1.00);
            ...
            entry.getValue().entrySet().stream().forEach(e->{
                System.out.println(String.format("-%s -> damage: %d, health: %d, armor: %d",e.getKey(),e.getValue()[0],
